...
beforeAll(async () => {
  // Get password from environment (use MYSQL_PASSWORD to match production config)
  const password = process.env.MYSQL_PASSWORD || process.env.DB_PASSWORD || 'Nk1865!.';
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      const [classResult] = await connection.execute(
        'INSERT INTO classes (name, major_id, `group`, year, semester, academic_year, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())',
        [testClass.name, testClass.majorId, testClass.group, testClass.year, testClass.semester, testClass.academicYear]
      ) as any;
