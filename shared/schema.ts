...
export const insertClassSchema = createInsertSchema(classes, {
  name: z.string().min(1, "Class name is required").optional(), // Auto-generated by backend
...
export const insertSubjectSchema = createInsertSchema(subjects, {
  ...
  credits: z.preprocess(
    (val) => val === null || val === undefined || val === "" ? 3 : val,
    z.number().min(1)
  ),
}).omit({ createdAt: true, updatedAt: true });
...
export const insertScheduleSchema = createInsertSchema(schedules, {
  ...
  startTime: z.string().regex(/^\d{2}:\d{2}(:\d{2})?$/, "Format: HH:MM or HH:MM:SS").transform(val => {
    // Convert HH:MM to HH:MM:SS if needed
    return val.length === 5 ? `${val}:00` : val;
  }),
  endTime: z.string().regex(/^\d{2}:\d{2}(:\d{2})?$/, "Format: HH:MM or HH:MM:SS").transform(val => {
    // Convert HH:MM to HH:MM:SS if needed
    return val.length === 5 ? `${val}:00` : val;
  }),
  room: z.string().nullish().transform(val => val || "TBA"), // Handle null/undefined
}).omit({ createdAt: true, updatedAt: true });

export const updateScheduleSchema = z.object({
  classId: z.number().min(1, "Class is required").optional(),
  subjectId: z.number().min(1, "Subject is required").optional(),
  teacherId: z.number().min(1, "Teacher is required").optional(),
  day: z.enum(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]).optional(),
  startTime: z.string().regex(/^\d{2}:\d{2}(:\d{2})?$/, "Format: HH:MM or HH:MM:SS").transform(val => {
    return val && val.length === 5 ? `${val}:00` : val;
  }).optional(),
  endTime: z.string().regex(/^\d{2}:\d{2}(:\d{2})?$/, "Format: HH:MM or HH:MM:SS").transform(val => {
    return val && val.length === 5 ? `${val}:00` : val;
  }).optional(),
  room: z.string().nullish().transform(val => val || "TBA").optional(),
});
...
export const insertLeaveRequestSchema = z.object({
  ...
  endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Format: YYYY-MM-DD"),
  reason: z.preprocess(
    (val) => val === null || val === undefined || val === "" ? "N/A" : val,
